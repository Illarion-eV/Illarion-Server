...
#include "Random.h"
#include "CSchedulerTaskClasses.h"
...
void CPlayer::login() throw(LogoutException)
{
        ...
        for ( int i = 0; i < RANGEUP; ++i ) 
        {
            if ( _world->GetPToCFieldAt( testf, pos.x, pos.y, pos.z + 1 + i, _world->tmap ) ) 
            {
                ...
                {
                    // Verdeckung des Spielers hat sich geï¿½dert
                    roofmap[ i ] = _world->tmap;
                    under[ i ] = ( testf->getTileId() != TRANSPARENT );
                } 
                else if ( _world->tmap != roofmap[ i ] ) 
                {
                // Spieler steht unter einer anderen Karte, ohne daï¿½sich die Verdeckung
                // geï¿½dert hat
                   roofmap[ i ] = _world->tmap;
                   under[ i ] = ( testf->getTileId() != TRANSPARENT );
                }
...
void CPlayer::SetAlive( bool t ) {
        ...
        else if ( lifestate & 1 ) 
        {
             ...
             {
                 PlayerDeath(); //Einzige ï¿½derung zum Orginal um Inventar zu droppen
                 ReduceSkills();
             }
        }
}
