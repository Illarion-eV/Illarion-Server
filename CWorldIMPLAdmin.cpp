...
bool CWorld::reload_defs( CPlayer * cp ) {
    ...
    // CScriptVariablesTable * ScriptVar_temp = 0;
    

	// load each table into a temporary var, if anything goes wrong these are just deleted
...
/*    
    if (ok) {
        ScriptVar_temp = new CScriptVariablesTable();
        if (ScriptVar_temp == NULL || !ScriptVar_temp->isDataOk())
        {
            reportTableError( cp, "scriptvariables" );
		    ok = false;
        }
    }
*/
