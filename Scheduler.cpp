...
void Scheduler::NextCycle() {
    ...
    if (!Tasks.empty()) {
        std::cout<<"Cycle anfang:"<<cycle<<" Liste front: "<<Tasks.front()->GetNextCycle()<<"\n";
    } else {
        std::cout<<"Cycle anfang:"<<cycle<<" Taskliste leer ";
    }
    ...
    while (!Tasks.empty() && (emexit < 201) && (Tasks.front()->GetNextCycle() <= cycle)) {
        ...
        if (!Tasks.empty()) {
            m_object = Tasks.front();    //obersten Task in Zeiger schieben
        }
        ...
        if (m_object!=NULL) {
            ...
            if ((*m_object)(world)) {
                AddTask(m_object);    //Wenn ausführen der Funktion true dann Task erneut adden
            }
        }
