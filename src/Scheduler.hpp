...
class ClockBasedScheduler {
	public:
		...
		void addRecurringTask(std::function<void()> task, std::chrono::nanoseconds interval, const std::string& taskname, bool start_immediately = false);
		void addRecurringTask(std::function<void()> task, std::chrono::nanoseconds interval, typename clock_type::time_point first_time, const std::string& taskname);
