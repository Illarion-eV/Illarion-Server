...
BBSendSkillTC::BBSendSkillTC(TYPE_OF_CHARACTER_ID id, TYPE_OF_SKILL_ID skill, short int value, short int minor) : BasicServerCommand(BB_SENDSKILL_TC) {
    addIntToBuffer(id);
...
BBSendAttribTC::BBSendAttribTC(TYPE_OF_CHARACTER_ID id, const std::string &attribute, short int value) : BasicServerCommand(BB_SENDATTRIB_TC) {
    addIntToBuffer(id);
...
BBPlayerTC::BBPlayerTC(TYPE_OF_CHARACTER_ID id, const std::string &name, const position &pos) : BasicServerCommand(BB_PLAYER_TC) {
    addIntToBuffer(id);
